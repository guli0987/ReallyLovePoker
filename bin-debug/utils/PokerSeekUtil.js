 ...
 */
var PokerSeekUtils = (function () {
    function PokerSeekUtils() {
    }
    /**
     * TODO
     * 从一手牌中判断有没有比指定牌大的牌
     ...
     * b 指定的牌形
     */
    PokerSeekUtils.seekRight = function (aHandPoker, pokerType) {
        var myPoker = PokerUtils.sortDescPokers(aHandPoker);
        var poker = PokerUtils.sortDescPokers(pokerType);
        console.log("seekRight", myPoker, poker);
        //找到别人扑克类型
        var bType = PokerTypeUtils.getType(pokerType);
        var bType2 = PokerTypeUtils.getSpecialType(pokerType);
        var mask = 0;
        var rightPoker = null;
        if (bType != null && bType2 == null) {
            if (bType.getType() === PokerTypeUtils.typeKingBoom)
                rightPoker = null;
            else if (bType.getType() === PokerTypeUtils.typeBoom)
                rightPoker = this.seekBoom(myPoker, bType.getSort(), pokerType.length);
            else if (bType.getType() === PokerTypeUtils.typeSingle)
                rightPoker = this.seekSingle(myPoker, bType.getSort());
            else if (bType.getType() === PokerTypeUtils.typePair)
                rightPoker = this.seekPairs(myPoker, bType.getSort());
            else if (bType.getType() === PokerTypeUtils.typeThree)
                rightPoker = this.seekThree(myPoker, bType.getSort());
            else if (bType.getType() === PokerTypeUtils.typeThreePairOn)
                rightPoker = this.seekThreePairOn(myPoker, bType.getSort());
            else if (bType.getType() === PokerTypeUtils.typeThreePairUnder)
                rightPoker = this.seekThreePairUnder(myPoker, bType.getSort());
            else if (bType.getType() === PokerTypeUtils.typeStraightPairs)
                rightPoker = this.seekStraightPairs(myPoker, bType.getSort(), poker.length / 2);
            else if (bType.getType() === PokerTypeUtils.typePlane)
                rightPoker = this.seekPlane(myPoker, bType.getSort(), poker.length / 2);
            else if (bType.getType() === PokerTypeUtils.typePlaneNPairsConnection2233)
                rightPoker = this.seekPlaneNPairsConnection2233(myPoker, bType.getSort(), pokerType.length / 5);
            else if (bType.getType() === PokerTypeUtils.typePlaneNPairsConnection3322)
                rightPoker = this.seekPlaneNPairsConnection3322(myPoker, bType.getSort(), pokerType.length / 5);
            else if (bType.getType() === PokerTypeUtils.typePlaneNPairsConnection2332)
                rightPoker = this.seekPlaneNPairsConnection2332(myPoker, bType.getSort());
        }
        //console.log("常用类型检索完");
        if (bType == null && bType2 != null) {
            console.log("找特殊类型");
            if (bType2.getType() === PokerTypeUtils.typeThreePair)
                rightPoker = this.seekThreePair(myPoker, bType2.getSort());
            else if (bType2.getType() === PokerTypeUtils.typePlaneNPairs)
                rightPoker = this.seekPlaneNPairs(myPoker, bType2.getSort(), pokerType.length / 5);
            else if (bType2.getType() === PokerTypeUtils.typePlaneNPairsConnection)
                rightPoker = this.seekPlaneNPairsConnection(myPoker, bType2.getSort(), pokerType.length / 5);
        }
        if (bType == null && bType2 == null) {
            //rightPoker = null;
            console.log("没有找到类型");
        ...
        if (rightPoker == null || rightPoker.length == 0) {
            ...
            if (enableOut) {
                var randomPress = Math.floor(Math.random() * 3 + 1);
                if (randomPress % 2 != 0) {
     ...
     * boomLength 炸弹的长度
     */
    PokerSeekUtils.seekBoom = function (aHandPokerString, typeSort, boomLength) {
        console.log("找呀找炸弹");
        ...
        for (var j = 0; j < aHandPokerString.length; j++) {
     ...
     * TODO 是否拆牌
     */
    PokerSeekUtils.seekSingle = function (aHandPokerString, typeSort) {
        console.log("找呀找单张");
        //从小到大查找能压住的牌
        var signleArray = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
            signleArray.push(aHandPokerString[j].getOrderValue());
        }
        for (var i = signleArray.length - 1; i >= 0; i--) {
            var endLength = signleArray.lastIndexOf(signleArray[i]) - signleArray.indexOf(signleArray[i]); //9876633
     ...
     * typeSort 要压住的牌的排序值
     */
    PokerSeekUtils.seekPairs = function (aHandPokerString, typeSort) {
        console.log("找呀找对子");
        ...
        for (var i = pairsArray.length - 1; i >= 0; i--) {
            var endLength = pairsArray.lastIndexOf(pairsArray[i]) - pairsArray.indexOf(pairsArray[i]); //9876544
            if (endLength === 1 && pairsArray[i] > typeSort) {
                return [aHandPokerString[i - 1], aHandPokerString[i]];
            }
        }
        for (var i = pairsArray.length - 1; i >= 0; i--) {
            var endLength = pairsArray.lastIndexOf(pairsArray[i]) - pairsArray.indexOf(pairsArray[i]); //11255556,012 3456 7
     ...
     * typeSort 要压住的牌的排序值
     */
    PokerSeekUtils.seekThree = function (aHandPokerString, typeSort) {
        console.log("找呀找三张");
        for (var j = aHandPokerString.length - 1; j >= 2; j--) {
     ...
     * typeSort 要压住的牌的排序值
     */
    PokerSeekUtils.seekThreePair = function (aHandPokerString, typeSort) {
        console.log("找呀找乱靠");
        var index = 0;
     ...
     * typeSort 要压住的牌的排序值
     */
    PokerSeekUtils.seekThreePairOn = function (aHandPokerString, typeSort) {
        console.log("找呀找上靠");
        for (var i = aHandPokerString.length - 1; i >= 4; i--) {
     ...
     * typeSort 要压住的牌的排序值
     */
    PokerSeekUtils.seekThreePairUnder = function (aHandPokerString, typeSort) {
        console.log("找呀找下靠");
        for (var i = aHandPokerString.length - 1; i >= 4; i--) {
            if (aHandPokerString[i].getOrderValue() === aHandPokerString[i - 1].getOrderValue()
     ...
     * length 要压住的顺子的长度
     */
    PokerSeekUtils.seekStraightPairs = function (aHandPokerString, typeSort, length) {
        console.log("找呀找连对");
        var returnPoker = new Array();
        var minPokerValue = typeSort - length + 2;
        var straightPairsArray = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
            straightPairsArray.push(aHandPokerString[j].getOrderValue());
        }
        for (var i = straightPairsArray.length - 1; i >= 0; i--) {
            var judgeSsp = true;
            if (straightPairsArray[i] >= minPokerValue) {
                for (var k = 0; k < length; k++) {
                    var indexSsp = straightPairsArray[i] + k;
                    if (indexSsp > 12) {
                        judgeSsp = false;
                        break;
                    }
            ...
            if (judgeSsp && straightPairsArray[i] >= minPokerValue) {
                for (var h = 0; h < length; h++) {
                    var indexReturn = straightPairsArray.lastIndexOf(straightPairsArray[i] + h);
                    //
     ...
     * 查找能压飞机的牌形算法√
     */
    PokerSeekUtils.seekPlane = function (aHandPokerString, typeSort, PlaneLength) {
        console.log("找呀找飞机");
        var returnPoker = new Array();
        var minPokerValue = typeSort - PlaneLength + 2;
        ...
        for (var i = PlaneArray.length - 1; i >= 0; i--) {
            ...
            if (PlaneArray[i] >= minPokerValue) {
                for (var k = 0; k < PlaneLength; k++) {
                    var indexSsp = PlaneArray[i] + k;
                    if (PlaneArray.lastIndexOf(indexSsp) - PlaneArray.indexOf(indexSsp) > 1) {
                        judgeSp = true;
                    }
                    else {
                        judgeSp = false;
            ...
            if (judgeSp && PlaneArray[i] >= minPokerValue) {
                for (var h = 0; h < PlaneLength; h++) {
                    var indexReturn = PlaneArray.lastIndexOf(PlaneArray[i] + h);
                    //
                    returnPoker.push(aHandPokerString[indexReturn]);
                    returnPoker.push(aHandPokerString[indexReturn - 1]);
     ...
     * 查找能压飞机带对子(无序)的牌形算法√
     */
    PokerSeekUtils.seekPlaneNPairs = function (aHandPokerString, typeSort, planeNlength) {
        console.log("找呀找飞机带对子");
        var returnPoker = new Array();
        var PlaneNPairsArray = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
            PlaneNPairsArray.push(aHandPokerString[j].getOrderValue());
        }
        //先找三张符合要求的，不符合直接返回null
        for (var i = PlaneNPairsArray.length - 1; i >= 0; i--) {
            var judgePnp = true;
            if (PlaneNPairsArray[i] > typeSort[typeSort.length - 1].getThreeValue()) {
                for (var k = 0; k < planeNlength; k++) {
                    var indexSsp = PlaneNPairsArray[i] + k;
                    if (PlaneNPairsArray.lastIndexOf(indexSsp) - PlaneNPairsArray.indexOf(indexSsp) > 1) {
                        judgePnp = true;
                    }
                    else {
                        judgePnp = false;
            ...
            if (judgePnp && PlaneNPairsArray[i] >= typeSort[typeSort.length - 1].getThreeValue()) {
                for (var h = 0; h < planeNlength; h++) {
                    var indexReturn = PlaneNPairsArray.lastIndexOf(PlaneNPairsArray[i] + h);
                    returnPoker.push(aHandPokerString[indexReturn]);
                    returnPoker.push(aHandPokerString[indexReturn - 1]);
                    returnPoker.push(aHandPokerString[indexReturn - 2]);
                }
        ...
        var typeSortIndex = typeSort.length - 1;
        for (var i = PlaneNPairsArray.length - 1; i >= 0; i--) {
            var judgePnp2 = true;
            if (PlaneNPairsArray[i] > typeSort[typeSortIndex].getTwoValue()) {
                var indexSsp2 = PlaneNPairsArray[i];
            ...
            if (judgePnp2 && PlaneNPairsArray[i] >= typeSort[typeSortIndex].getTwoValue()) {
                var indexReturn2 = PlaneNPairsArray.lastIndexOf(PlaneNPairsArray[i]);
     ...
     */
    PokerSeekUtils.seekPlaneNPairsConnection = function (aHandPokerString, typeSort, planeNlength) {
        console.log("找呀找飞机带对子");
        var returnPoker = new Array();
        var PlaneNPairsArray = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
        ...
        for (var i = PlaneNPairsArray.length - 1; i >= 0; i--) {
            ...
            if (PlaneNPairsArray[i] > typeSort[typeSort.length - 1].getTwoValue()) {
                for (var k = 0; k < planeNlength; k++) {
                    var indexSsp2 = PlaneNPairsArray[i] + k;
                    if (PlaneNPairsArray.lastIndexOf(indexSsp2) - PlaneNPairsArray.indexOf(indexSsp2) === 1) {
                        judgePnp2 = true;
                    }
                    else {
            ...
            if (judgePnp2 && PlaneNPairsArray[i] >= typeSort[typeSort.length - 1].getTwoValue()) {
                for (var h = 0; h < planeNlength; h++) {
                    var indexReturn2 = PlaneNPairsArray.lastIndexOf(PlaneNPairsArray[i] + h);
                    returnPoker.push(aHandPokerString[indexReturn2]);
                    returnPoker.push(aHandPokerString[indexReturn2 - 1]);
                }
                break;
     ...
     */
    PokerSeekUtils.seekPlaneNPairsConnection2233 = function (aHandPokerString, typeSort, planeNlength2233) {
        ...
        var PlaneNPairsArray2233 = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
            PlaneNPairsArray2233.push(aHandPokerString[j].getOrderValue());
        }
        //先找对子符合长度的
        for (var i = PlaneNPairsArray2233.length - 1; i >= 0; i--) {
            var judjePnp2233Two = true;
            var judjePnp2233Three = true;
            //9998887777  6  6  5  5  4  3  3
            //0123456789 10 11 12 13 14 15 16
            if (PlaneNPairsArray2233[i] > typeSort) {
                for (var k = 0; k < planeNlength2233; k++) {
                    var indexSsp2233Two = PlaneNPairsArray2233[i] + k;
                ...
                if (judjePnp2233Two) {
                    for (var k = 0; k < planeNlength2233; k++) {
                        var indexSsp2233Three = PlaneNPairsArray2233[i] - planeNlength2233 * 2;
                ...
                if (judjePnp2233Two && judjePnp2233Three) {
                    for (var h = 0; h < planeNlength2233; h++) {
                        var indexReturn2233Two = PlaneNPairsArray2233.lastIndexOf(PlaneNPairsArray2233[i] + h);
                        returnPoker.push(aHandPokerString[indexReturn2233Two]);
                        returnPoker.push(aHandPokerString[indexReturn2233Two - 1]);
                    }
                    for (var h = 0; h < planeNlength2233; h++) {
                        var indexReturn2233Three = PlaneNPairsArray2233.lastIndexOf(PlaneNPairsArray2233[i] - planeNlength2233 * 2 + h);
     ...
     */
    PokerSeekUtils.seekPlaneNPairsConnection3322 = function (aHandPokerString, typeSort, planeNlength3322) {
        console.log("找呀找飞机带对子(有序3322)");
        var returnPoker = new Array();
        //let returnThreePoker: Array<Poker> =new Array<Poker>();
        //let returnTwoPoker: Array<Poker> =new Array<Poker>();
        var PlaneNPairsArray3322 = new Array();
        for (var j = 0; j < aHandPokerString.length; j++) {
            PlaneNPairsArray3322.push(aHandPokerString[j].getOrderValue());
        }
        //先找三张符合长度的
        for (var i = PlaneNPairsArray3322.length - 1; i >= 0; i--) {
            var judjePnp2233Two = true;
            var judjePnp2233Three = true;
            //9998887777  6  6  5  5  4  3  3
            //0123456789 10 11 12 13 14 15 16
            if (PlaneNPairsArray3322[i] > typeSort) {
                for (var k = 0; k < planeNlength3322; k++) {
                    var indexSsp3322Three = PlaneNPairsArray3322[i] + k;
                    var threeIndexEnd = PlaneNPairsArray3322.lastIndexOf(indexSsp3322Three) - PlaneNPairsArray3322.indexOf(indexSsp3322Three);
                ...
                if (judjePnp2233Three) {
                    for (var k = 0; k < planeNlength3322; k++) {
                        var indexSsp2233Two = PlaneNPairsArray3322[i] - planeNlength3322 * 3;
                ...
                //如果都为真则条件成立
                if (judjePnp2233Two && judjePnp2233Three) {
                    for (var h = 0; h < planeNlength3322; h++) {
                        var indexReturn3322Three = PlaneNPairsArray3322.lastIndexOf(PlaneNPairsArray3322[i] + h);
                    ...
                    for (var h = 0; h < planeNlength3322; h++) {
                        var indexReturn2233Two = PlaneNPairsArray3322.lastIndexOf(PlaneNPairsArray3322[i] - planeNlength3322 * 3 + h);
                        returnPoker.push(aHandPokerString[indexReturn2233Two]);
                        returnPoker.push(aHandPokerString[indexReturn2233Two - 1]);
                    }
                    break;
     ...
     */
    PokerSeekUtils.seekPlaneNPairsConnection2332 = function (aHandPokerString, typeSort) {
    ...
    PokerSeekUtils.randomPlay = function (myPoker) {
        ...
        if (myPoker.length >= 15) {
        ...
        else if (myPoker.length < 15 && myPoker.length > 5) {
            console.log("7<牌数<15");
            //如果自己有单张，优先出
            for (var i = numberArray.length - 1; i >= 0; i--) {
                if (numberArray.indexOf(numberArray[i]) - numberArray.lastIndexOf(numberArray[i]) == 0) {
            ...
            if (rememberI != -1)
                return [myPoker[rememberI], myPoker[rememberI - 1], myPoker[rememberI - 1]];
            ...
            if (singleCount == 3) {
                ...
                var random1Or0 = Math.floor(Math.random() * 2);
                if (random1Or0 == 0) {
     ...
     * seat 出牌玩家的座位号
     */
    PokerSeekUtils.autoOutSinglePlay = function (myPoker) {
     ...
     * seat 出牌玩家的座位号
     */
    PokerSeekUtils.autoThrowPlay = function (myPoker) {
     ...
     * seat 出牌玩家的座位号
     */
    PokerSeekUtils.autoPrompt = function (myPoker, mySeat, isCampNumber, poker, seat) {
     ...
     * seat 出牌玩家的座位号
     */
    PokerSeekUtils.autoPromptOnline = function (myPoker, poker) {
        //myPoker = PokerUtils.sortDescPokers(myPoker);//避免右手握牌带来问题
     ...
     * seat 出牌玩家的座位号
     */
    PokerSeekUtils.autoPlay = function (myPoker, mySeat, isCampNumber, poker, seat) {
        ...
        //如果在同一阵营，暂定为不要
        if (seat === isCampNumber) {
            if (PokerTypeUtils.isSingle(poker) != -1) {
            ...
            else if (PokerTypeUtils.isPair(poker) != -1) {
                if (poker[0].getOrderValue() < 10) {
                    return this.seekRight(myPoker, poker);
                }
            }
            else if (PokerTypeUtils.isThree(poker) != -1) {
                if (poker[0].getOrderValue() < 12) {
                    return this.seekRight(myPoker, poker);
                }
            }
            else if (PokerTypeUtils.isStraightPairs(poker) != -1) {
    ...
    return PokerSeekUtils;
}());
__reflect(PokerSeekUtils.prototype, "PokerSeekUtils");
