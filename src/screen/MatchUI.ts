class MatchUI extends eui.Component implements  eui.UIComponent {
     ...
     * @param tableID 
     */
    private addPlayerList(id:number,name:string,avatar:string, tableID:number, isowner:boolean = false):GameUser{
    ...
    private showPlayer(player:GameUser){
        ...
        }else{//不是创建房间和根据房间号查找，开始游戏和地图不可见
    ...
    private leaveRoomNotify(ev:egret.Event) {
         ...
         * 是否房主有变动，有变动的话就转移房主
         */
        if(data.owner == GameData.gameUser.id){//如果房主为自己
            this.isOwner = true;
        }else{
            this.isOwner = false;
        }
